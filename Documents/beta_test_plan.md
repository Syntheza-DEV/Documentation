...
Contrairement aux plateformes fondées sur l’économie de l’attention, Syntheza se positionne sur le créneau du **« Slow Content »** productif. Le produit cible prioritairement le segment des **« Prosumers »** :

* **Étudiants en fin de cursus** : sourcer rapidement mémoires et projets.
...
| N° | Tâche | Fonctionnalité(s) | Impact stratégique / business |
|----|-------|-------------------|-------------------------------|
...
| 3 | Auth backend (Inscription/Connexion + JWT) | F1, F2 | Sécurisation de l’accès et isolation des données utilisateurs. |
...
| 9 | Pipeline tri articles (Ranking V1) | F9 | Élimination de l’addiction au scroll (tri par pertinence). |
| 10 | Matching utilisateur ↔ articles | F9 | Hyper-pertinence du feed, réduction du churn des utilisateurs exigeants. |
| 11 | Résumé automatique articles | F12 | Automatisation de la valeur, réduction de la charge cognitive à l’ouverture. |
| 12 | Trust Factor V1 | F14 | Différenciation concurrentielle majeure (lutte anti-désinformation). |
| 13 | Exposition Trust Factor API | F14 | Disponibilité multi-plateforme de la brique de confiance. |
| 14 | Benchmark solutions LLM | F12, F18 | Maîtrise des coûts serveurs et optimisation de la latence. |
| 15 | Endpoint Feed unifié | F9 | Centralisation multi-sources unifiant l’expérience de veille. |
| 16 | Like backend | F15 | Premier niveau d’engagement et récolte de signaux d’intérêt. |
| 17 | Commentaires backend | F15 | Rétention sociale, animation de la communauté de veilleurs. |
| 18 | Password Lost Reset API | F3 | Réduction de la friction de reconnexion, rétention des utilisateurs actifs. |
| 19 | Recherche articles V1 | F11 | Outil de productivité pour retrouver une info archivée ou chaude. |
| 20 | Configuration sources backend | F7 | Flexibilité du catalogue de veille, adaptation aux besoins sectoriels. |
| 21 | Stabilisation backend | F18 | Fiabilité technique indispensable à la crédibilité B2B. |
| 22 | Page Password Lost complète (backend) | F3 | Autonomie de l’utilisateur, diminution des demandes de support. |
| 23 | Composant Trust Factor (web) | F14 | Transparence algorithmique immédiate pour rassurer le professionnel. |
| 24 | Configuration sources Settings (web) | F7 | Point d’entrée de la personnalisation de la veille. |
| 25 | Pagination infinite scroll Feed/Discover (web) | F10 | Confort de navigation sans rechargement, contrôle du flux. |
| 26 | Bookmark posts (web) | F15 | Bibliothèque de connaissances personnelle, valeur d’usage long terme. |
| 27 | Recherche Search onglet manquant (web) | F11 | Complétude de l’expérience bureau pour une recherche transversale. |
| 28 | Follow/Unfollow (web) | F16 | Mécanisme de viralité in-app (suivre les experts de son secteur). |
| 29 | Upload photo profil (web) | F16 | Personnalisation de l’identité visuelle sur la plateforme. |
| 30 | Changement mot de passe Settings (web) | F3 | Sécurité du compte de bout en bout depuis l’espace personnel. |
| 31 | Navigation mobile UI (web responsive) | F18 | Accessibilité cross-device pour l’utilisateur en déplacement. |
| 32 | Responsive mobile global (web) | F18 | Uniformisation de l’image de marque multi-écrans. |
| 33 | API Password Reset (web) | F3 | Sécurisation du protocole de récupération de compte. |
| 34 | API Trust Factor (web) | F14 | Intégration de l’affichage du score de confiance côté web. |
| 35 | API Sources (web) | F6, F7 | Synchronisation des ajouts de flux RSS depuis le tableau de bord. |
| 36 | Protection routes auth (web) | F4 | Sécurisation de l’espace abonné. |
| 37 | Gestion token refresh (web) | F4 | Rétention : expérience fluide sans reconnexions. |
| 38 | Gestion erreurs globales UI (web) | F18 | Maintien de l’image de marque (zéro écran blanc) en cas d’anomalie. |
| 39 | Logout complet (web) | F5 | Confidentialité des sessions sur ordinateurs partagés. |
| 40 | Auth mobile | F1, F2, F4 | Point d’entrée clé du mobile, sécurisation de la session smartphone. |
| 41 | Password Lost mobile | F3 | Autonomie de récupération de compte depuis le mobile. |
| 42 | Feed mobile | F9 | Cœur de l’usage nomade, consultation rapide de l’actualité filtrée. |
| 43 | Infinite scroll mobile | F10 | Ergonomie mobile fluide pour l’analyse rapide des titres. |
| 44 | Discover mobile | F10 | Moteur de découverte de nouvelles sources pour enrichir sa veille. |
| 45 | Search mobile | F11 | Accès immédiat à l’information ciblée en mobilité. |
| 46 | Settings mobile | F7, F3, F16 | Gestion du compte et des préférences de veille sur mobile. |
| 47 | Sources mobile | F6, F7 | Ajout d’un flux RSS directement depuis le mobile. |
| 48 | Like mobile | F15 | Interaction tactile rapide facilitant le feedback instantané. |
| 49 | Commentaires mobile | F15 | Participation aux débats sectoriels depuis le smartphone. |
| 50 | Trust Factor mobile | F14 | Décision instantanée en mobilité : fiabilité de l’info en un coup d’œil. |
| 51 | Bookmark mobile | F15 | Sauvegarde d’un article pour une lecture approfondie ultérieure. |
| 52 | Follow mobile | F16 | Extension du réseau professionnel in-app depuis le mobile. |
| 53 | Subscribe mobile | F7 | Activation du suivi de nouveaux flux sur mobile. |
| 54 | Upload avatar mobile | F16 | Appareil photo/galerie pour humaniser le profil. |
| 55 | Password change mobile | F3 | Modification rapide et sécurisée des identifiants. |
| 56 | Profile mobile | F16 | Vitrine personnelle synthétisant les centres d’intérêt de veille. |
| 57 | Connexion prod mobile | F2 | Test en conditions réelles sur le serveur de production. |
| 58 | UX/UI mobile | F18 | Confort visuel (Slow Content) limitant la fatigue cognitive. |
| 59 | Stabilisation Beta mobile | F18 | Réduction des crashs (KPI clé de rétention en bêta). |
| 60 | Ingestion Twitter stub | F8 | Preuve de concept pour l’extension future vers les réseaux sociaux. |
| 61 | Notifications in-app | F17 | Réengagement de la cible Prosumer sur les urgences de veille. |
| 62 | Observabilité logs | F18 | Supervision de l’infra, détection proactive des anomalies. |
| 63 | Documentation technique MVP | F18 | Alignement de l’équipe et pérennité du code. |
